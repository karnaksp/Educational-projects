...
answ = redis.Redis(db=0)
visits = redis.Redis(db=1)
...
class Task(pydantic.BaseModel):
    ...
    async def process_url(self, idx: int, session: ClientSession):
        cache = await answ.get(str(self.urls[idx]))
        if cache:
            self.result[idx] = int(cache)
            # print(cache)
        else:
            async with session.get(str(self.urls[idx])) as response:
                self.result[idx] = response.status
                await answ.set(str(self.urls[idx]), response.status)
        cache_visits = await visits.get(self.urls[idx].host)
        if cache_visits:
            # print(cache_visits)
            await visits.set(self.urls[idx].host, int(cache_visits) + 1)
        else:
            await visits.set(self.urls[idx].host, 1)
